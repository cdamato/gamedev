...
void resize_ui(engine& g, f32 scale_factor) {
    auto& widget = g.ecs.get<ecs::widget>(g.ui.root);
...
int cursorpos_to_navindex(entity e, engine& g, sprite_coords ev_pos) {
    if (!g.ecs.exists<ecs::selection>(e)) {
        return 0;
    }
    auto& select = g.ecs.get<ecs::selection>(e);
    rect<f32> grid_entry = g.ecs.get<ecs::display>(e).sprites(0).get_dimensions(0);
    point<u16> cursor_focus = [&] () {;
        if (!test_collision(g.ecs.get<ecs::display>(e).sprites(0).get_dimensions(), ev_pos)) {
    ...
    if (g.ecs.exists<ecs::checkbox>(e)) {
        ...
        auto& select = g.ecs.get<ecs::selection>(e);
        if (focus_index < select.num_elements()) {
            if (test_collision(g.ecs.get<ecs::display>(e).sprites(dropdown.sprite_index).get_dimensions(focus_index), ev_pos)) {
                return focus_index;
            } else {
                return 65535;
            }
...
entity at_cursor(entity e, engine& g, screen_coords cursor) {
    entity next = 65535;
     ecs::widget& w = g.ecs.get<ecs::widget>(e);
    for (auto it = w.children.begin(); it != w.children.end(); it++) {
        if (g.ecs.get<ecs::display>(*it).sprites(0).layer == render_layers::sprites)
            continue;
        if (test_collision(g.ecs.get<ecs::display>(*it).get_dimensions(), cursor.to<f32>()))
            next = *it;
    }
...
bool send_actionevent(entity e, engine& g, bool release) {
    if (e != 65535 && e != g.ui.root) {
        g.ui.hover_timer.start();
        g.ecs.get<ecs::widget>(e).on_activate(e, g, release);
        return true;
    }
    return false;
}
...
bool handle_button(input_event& e_in, engine& g) {
    ...
    if (g.ecs.get<ecs::widget>(dest).accepts_textinput) {
        screen_coords position = g.ui.last_position - g.ecs.get<ecs::display>(dest).sprites(0).get_dimensions().origin.to<u16>();
        textinput_navigation(dest, g, g.ecs.systems.text.character_at_position(g.ecs.get<ecs::text>(dest).text_entries.begin()->text, position));
        return true;
    }
    while (g.ecs.get<ecs::widget>(dest).on_activate == nullptr) {
        dest = g.ecs.get<ecs::widget>(dest).parent;
    ...
    if (ev.release() && held != dest && held != 65535) {
         ecs::widget::activation_action function = g.ecs.get<ecs::widget>(held).on_activate;
        if (function != nullptr) {
    ...
    if (ev.release() && g.in_dungeon) {
         ecs::player& p = g.ecs.get<ecs::player>(g.player_id());
        p.shoot = true;
        p.target = h + g.offset;
        return true;
    }
...
bool handle_keypress(input_event& e_in, engine& g) {
    ...
    if (it == g.settings.bindings.end()) {
        ...
        return false;
        // return g.ecs.get<ecs::event_callbacks>(g.ui.focus).run_event(e);
    }
    ...
    switch (command) {
        case command::interact: {
            ...
            g.ecs.get<ecs::widget>(active).on_activate(active, g, e.release());
            break;
        }
        case command::toggle_inventory: {
            ...
            if (toggle_state) {
                ...
                g.create_entity(inventory_init, g.ui.root, g.ecs.get<ecs::inventory>(g.player_id()), point<u16>(100, 100));
            }
        ...
        case command::move_left:
            g.ecs.get<ecs::velocity>(g.player_id()).delta.x += e.release() ? 0.10 : -0.10;
            return true;
        case command::move_right:
            g.ecs.get<ecs::velocity>(g.player_id()).delta.x -= e.release() ? 0.10 : -0.10;
            return true;
        case command::move_up:
            g.ecs.get<ecs::velocity>(g.player_id()).delta.y += e.release() ? 0.10 : -0.10;
            return true;
        ...
        case command::nav_activate:
            g.ui.last_position = screen_coords(65535, 65535);
            return send_actionevent(g.ui.focus, g, e.release());
        case command::text_backspace:
            if (!e.release()) return false;
            if (g.ecs.get<ecs::widget>(g.ui.focus).accepts_textinput == true) {
                std::string thing = " ";
                thing[0] = 0x08;
...
bool handle_cursor(input_event& e_in, engine& g) {
    ...
    //if (g.ui.cursor != 65535) g.ecs.get<ecs::event_callbacks>(g.ui.cursor).run_event(e);
    ...
    if (start == dest)  {
        if (g.ecs.get<ecs::widget>(dest).accepts_textinput) return true;
...
bool handle_hover(input_event& e, engine& g) {
    ...
    //return g.ecs.get<ecs::event_callbacks>(dest).run_event(e);
}

bool handle_textinput(input_event& ev_in, engine& g) {
    auto& ev = dynamic_cast<event_textinput&>(ev_in);
    if (g.ui.focus == 65535) return false;
    if (g.ecs.get<ecs::widget>(g.ui.focus).accepts_textinput) {
...
void make_parent(entity a, entity b, engine& game) {
     ecs::widget& w_a = game.ecs.get<ecs::widget>(a);
     ecs::widget& w_b = game.ecs.get<ecs::widget>(b);
